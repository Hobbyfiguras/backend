...
class ThreadSearchSerializer(HaystackSerializer):
    class Meta:
        ...
        fields = ["text", "username", "slug", "nsfw", "thread_id", "forum",
                  "modified", "created", "post_count", "last_post_creator"]
...
class ThreadViewSet(ExternalIdViewMixin, mixins.UpdateModelMixin, mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        if page is not None:
            ...
            posts = serializers.PostSerializer(
                thread.posts, many=True, context={'request': request})
    ...
    def change_subscription(self, request, pk=None):
        ...
        if 'subscribed' in request.data:
            thread.change_user_subscription(
                request.user, request.data['subscribed'])
    ...
    def move_thread(self, request, pk=None):
        ...
        if 'forum' in request.data:
            forum = Forum.objects.get(
                id=internal_id_from_model_and_external_id(Forum, request.data['forum']))
            if forum:
                thread.forum = forum
                thread.save()
                return Response({}, status=status.HTTP_200_OK)
            ...
            for subscriber in thread.subscribers.all():
                ...
                if request.user.id != subscriber.id:
                    notification = Notification.objects.create(
                        notification_type="notification_post_sub", user=subscriber, actor=request.user, notification_object=serializer.instance)
                    notification.save()
